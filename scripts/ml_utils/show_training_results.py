...
experiments = sorted(
    [d for d in experiments_dir.iterdir() if d.is_dir()], key=lambda x: x.name, reverse=True
)
...
if summary_path.exists():
    ...
    print(
        f"{'Model':<25} {'CV Score':<12} {'Val Recall':<12} {'Val F1':<12} {'Composite':<12} {'Meets Criteria'}"
    )
    print(f"{'-'*100}")
    ...
    for model_name, results in summary.get("models", {}).items():
        ...
        print(
            f"{model_name:<25} {cv_score:<12.4f} {recall:<12.4f} {f1:<12.4f} {composite:<12.4f} {meets}"
        )
    ...
    if best_model_name and best_model_name in summary.get("models", {}):
        ...
        print(
            f"Meets Criteria: {'✅ YES' if best_results.get('meets_criteria', False) else '❌ NO'}"
        )
