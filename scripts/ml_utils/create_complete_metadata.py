...
for model_name, model_path in model_configs:
    ...
    if "smote" in model.named_steps:
        ...
        smote_config = {
            "sampling_strategy": (
                float(smote.sampling_strategy)
                if isinstance(smote.sampling_strategy, (int, float))
                else smote.sampling_strategy
            ),
    ...
    if model_name == "Logistic Regression":
        ...
        hyperparameters = {
            ...
            "max_features": (
                float(classifier.max_features)
                if isinstance(classifier.max_features, float)
                else str(classifier.max_features)
            ),
            "class_weight": str(classifier.class_weight),
    ...
    if hasattr(classifier, "coef_"):
        ...
        top_features = [
            {"feature": feature_names[i], "coefficient": float(coef[i])} for i in top_indices
        ]
        ...
        top_features = [
            {"feature": feature_names[i], "importance": float(importances[i])} for i in top_indices
        ]
    else:
        top_features = []
    ...
    val_optimal_threshold, _ = EvaluationCriteria.find_optimal_threshold(
        y_val, y_val_proba, target_recall=0.90
    )

    print("📊 VALIDATION SET METRICS:")
    print(
        f"  Recall: {val_metrics['recall']:.4f}, F1: {val_metrics['f1']:.4f}, ROC-AUC: {val_metrics['roc_auc']:.4f}"
    )
    ...
    test_optimal_threshold, _ = EvaluationCriteria.find_optimal_threshold(
        y_test, y_test_proba, target_recall=0.90
    )

    print("📊 TEST SET METRICS:")
    print(
        f"  Recall: {test_metrics['recall']:.4f}, F1: {test_metrics['f1']:.4f}, ROC-AUC: {test_metrics['roc_auc']:.4f}"
    )
...
print()
print(
    f"{'Model':<25} {'Val Recall':>10} {'Val F1':>10} {'Test Recall':>11} {'Test F1':>10} {'Best':>6}"
)
print("-" * 100)
...
for model_name, metadata in models_metadata.items():
    ...
    print(
        f"{model_name:<25} {val_recall:>10.4f} {val_f1:>10.4f} {test_recall:>11.4f} {test_f1:>10.4f} {is_best:>6}"
    )
