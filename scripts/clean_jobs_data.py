...
class JobDataCleaner:
    ...
    def __init__(self, input_path: str, output_path: str):
        ...
        self.stats = {"total": 0, "cleaned": 0, "skipped": 0, "errors": []}
    ...
    def clean_title(self, title: str) -> str:
        ...
        cleaned = re.sub(
            r"\(\s*(?:contract|permanent|temporary|part[- ]time|full[- ]time)\s*(?:to\s+hire)?\s*$",
            "",
            cleaned,
            flags=re.IGNORECASE,
        )
        cleaned = re.sub(
            r"\(\s*\w+\s+operation\s*$", "", cleaned, flags=re.IGNORECASE
        )  # (Banking Operation
    ...
    def extract_location(self, location_str: str, title: str) -> tuple:
        ...
        for check_city, check_country in self.GLOBAL_CITIES:
            if check_city.lower() in location_lower:
                ...
                remote_type = random.choices(["on-site", "hybrid", "remote"], weights=[50, 30, 20])[
                    0
                ]
                return city, country, remote_type
    ...
    def generate_description(
        self, title: str, required_skills: List[str], min_exp: int, seniority: str
    ) -> str:
        """Generate a realistic job description"""
        exp_text = f"{min_exp}+ years" if min_exp > 0 else "0-2 years"
        skills_text = (
            ", ".join(required_skills[:5]) if required_skills else "relevant technical skills"
        )
    ...
    def clean_job(
        self, job_dict: Dict[str, Any], index: int, total: int
        ...
        try:
            ...
            self.stats["errors"].append(
                f"Error processing job {job_dict.get('Job Id', 'unknown')}: {str(e)}"
            )
            return None
    ...
    def clean_all(self):
        ...
        print(
            f"Successfully cleaned: {self.stats['cleaned']} ({self.stats['cleaned']/self.stats['total']*100:.1f}%)"
        )
        print(
            f"Skipped (invalid):    {self.stats['skipped']} ({self.stats['skipped']/self.stats['total']*100:.1f}%)"
        )
        ...
        for i, job in enumerate(cleaned_jobs[:3], 1):
            ...
            print(
                f"   Experience: {job['min_experience_years']}-{job['max_experience_years']} years ({job['seniority_level']})"
            )
