...
class CVProfile(BaseModel):
    ...
    extracted_data: Dict[str, Any] = Field(
        default_factory=dict, description="Raw extraction results"
    )
...
class MatchResult(BaseModel):
    ...
    explanation_source: Optional[str] = Field(
        None, description="Provider that generated the explanation"
    )
...
def match_result_to_history(match: MatchResult) -> MatchHistory:
    ...
    return MatchHistory(
        ...
        candidate_skills=json.dumps(
            match.score_breakdown.matched_skills + match.score_breakdown.extra_skills
        ),
        required_skills=json.dumps(
            match.score_breakdown.matched_skills + match.score_breakdown.missing_skills
        ),
