...
class ATSModelTrainer:
    ...
    def train_with_grid_search(
        ...
        if use_randomized and candidates > n_iter:
            ...
            search = GridSearchCV(
                pipeline, param_grid=param_grid, cv=5, scoring="recall", n_jobs=-1, verbose=1
            )
    ...
    def _select_best_model(self):
        ...
        logger.info(
            f"{'Model':<20} {'Recall':>8} {'F1':>8} {'ROC-AUC':>8} {'Composite':>10} {'Meets Criteria':>15}"
        )
        logger.info(f"{'-'*80}")
        ...
        valid_models = {
            name: results for name, results in self.models.items() if results["meets_criteria"]
        }
