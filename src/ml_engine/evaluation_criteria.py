...
class EvaluationCriteria:
    ...
    WEIGHTS = {"recall": 0.40, "f1": 0.25, "roc_auc": 0.20, "precision": 0.10, "accuracy": 0.05}
    ...
    def print_evaluation_report(
        ...
        logger.info("\n📊 PRIMARY METRICS:")
        logger.info(
            f"  Recall (Sensitivity):    {metrics.get('recall', 0):.4f} {'✅' if metrics.get('recall', 0) >= EvaluationCriteria.THRESHOLDS['recall_min'] else '❌'}"
        )
        logger.info(
            f"  F1 Score:                {metrics.get('f1', 0):.4f} {'✅' if metrics.get('f1', 0) >= EvaluationCriteria.THRESHOLDS['f1_min'] else '❌'}"
        )
        logger.info(
            f"  ROC-AUC:                 {metrics.get('roc_auc', 0):.4f} {'✅' if metrics.get('roc_auc', 0) >= EvaluationCriteria.THRESHOLDS['roc_auc_min'] else '❌'}"
        )
        logger.info(
            f"  Precision:               {metrics.get('precision', 0):.4f} {'✅' if metrics.get('precision', 0) >= EvaluationCriteria.THRESHOLDS['precision_min'] else '❌'}"
        )
        logger.info(
            f"  Accuracy:                {metrics.get('accuracy', 0):.4f} {'✅' if metrics.get('accuracy', 0) >= EvaluationCriteria.THRESHOLDS['accuracy_min'] else '❌'}"
        )

        # Business metrics
        logger.info("\n💼 BUSINESS METRICS:")
        logger.info(
            f"  False Negative Rate:     {metrics.get('false_negative_rate', 0):.4f} (missed good candidates)"
        )
        logger.info(
            f"  False Positive Rate:     {metrics.get('false_positive_rate', 0):.4f} (wasted interviews)"
        )
        logger.info(
            f"  Specificity:             {metrics.get('specificity', 0):.4f} (true rejection rate)"
        )

        # Confusion matrix
        if all(
            k in metrics
            for k in ["true_positives", "true_negatives", "false_positives", "false_negatives"]
        ):
            logger.info("\n📋 CONFUSION MATRIX:")
            logger.info(
                f"  True Positives:  {metrics['true_positives']:4d} (correctly identified qualified)"
            )
            logger.info(
                f"  True Negatives:  {metrics['true_negatives']:4d} (correctly rejected unqualified)"
            )
            logger.info(f"  False Positives: {metrics['false_positives']:4d} (false alarms)")
            logger.info(
                f"  False Negatives: {metrics['false_negatives']:4d} (missed opportunities) ⚠️"
            )
