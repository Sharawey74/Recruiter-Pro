...
class FeatureEngineer:
    ...
    def _create_numerical_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        features["years_per_project"] = np.where(
            features["projects_count"] > 0, features["experience"] / features["projects_count"], 0
        )
    ...
    def fit_transform(
        self, df: pd.DataFrame, exclude_ai_score: bool = True
        ...
        skills_df = pd.DataFrame([self._parse_skills(text) for text in df["Skills"]]).reset_index(
            drop=True
        )
        ...
        education_df = pd.DataFrame(education_encoded, columns=["education_level"]).reset_index(
            drop=True
        )
        ...
        all_features = pd.concat(
            [skills_df, education_df, cert_dummies, role_dummies, numerical_features], axis=1
        )
        ...
        self.fitted = True
        logger.info(
            f"✅ Feature engineering complete. Shape: {X.shape}, Features: {len(self.feature_names)}"
        )
