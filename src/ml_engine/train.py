...
def main(args):
    ...
    if args.run_cv_analysis:
        ...
        cv_evaluator = CrossValidationEvaluator(
            n_folds=5, random_state=args.random_state, output_dir=experiment_dir
        )
        ...
        baseline_models = {
            "Logistic Regression": LogisticRegression(
                random_state=args.random_state, max_iter=1000
            ),
            "Random Forest": RandomForestClassifier(
                random_state=args.random_state, n_estimators=100
            ),
            "XGBoost": XGBClassifier(
                random_state=args.random_state, n_estimators=100, use_label_encoder=False
            ),
        }

        for model_name, model in baseline_models.items():
            # CV performance
            cv_results = cv_evaluator.evaluate_cv_performance(
                model, X_train, y_train, scoring="recall"
            )
    ...
    trainer = ATSModelTrainer(random_state=args.random_state, output_dir=experiment_dir)
    ...
    EvaluationCriteria.print_evaluation_report(
        f"{best_model_name} (Test Set)", test_metrics, y_test, y_test_pred
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data-path", type=str, default="resumes.csv", help="Path to the resume dataset CSV file"
    )
    ...
    parser.add_argument(
        "--test-size", type=float, default=0.15, help="Proportion of data for test set"
    )

    parser.add_argument(
        "--val-size", type=float, default=0.15, help="Proportion of data for validation set"
    )
