...
class ATSPredictor:
    ...
    def predict(
        self, cv_data: Union[Dict, pd.DataFrame], use_optimal_threshold: bool = True
    ) -> Dict:
        """
    ...
    def predict_batch(
        self, cv_data_list: Union[List[Dict], pd.DataFrame], use_optimal_threshold: bool = True
    ) -> List[Dict]:
        """
    ...
    def get_feature_importance(self, top_n: int = 20) -> Dict[str, float]:
        ...
        if len(feature_names) != len(importances):
            logger.warning(
                f"Feature name count mismatch: {len(feature_names)} vs {len(importances)}"
            )
...
def predict_resume(
    cv_data: Union[Dict, pd.DataFrame], model_dir: str = "models/production"
) -> Dict:
    """
