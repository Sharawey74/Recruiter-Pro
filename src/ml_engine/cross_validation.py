...
class CrossValidationEvaluator:
    ...
    def __init__(
        self, n_folds: int = 5, random_state: int = 42, output_dir: str = "models/experiments"
    ):
        """
    ...
    def evaluate_cv_performance(
        self, estimator: BaseEstimator, X: np.ndarray, y: np.ndarray, scoring: str = "recall"
    ) -> Dict[str, float]:
        """
        ...
        scores = cross_val_score(estimator, X, y, cv=self.cv_splitter, scoring=scoring, n_jobs=-1)
        ...
        if cv_stats["std"] > 0.1:
            logger.warning(
                f"⚠️  High variance detected (std={cv_stats['std']:.4f}). Possible overfitting!"
            )
    ...
    def plot_learning_curves(
        ...
        plt.fill_between(
            train_sizes_abs, val_mean - val_std, val_mean + val_std, alpha=0.1, color="red"
        )
        ...
        plot_path = os.path.join(
            self.output_dir, f"learning_curve_{model_name.replace(' ', '_')}.png"
        )
    ...
    def plot_validation_curve(
        ...
        # Save plot
        plot_path = os.path.join(
            self.output_dir, f"validation_curve_{model_name.replace(' ', '_')}_{param_name}.png"
        )
        plt.savefig(plot_path, dpi=150, bbox_inches="tight")
        plt.close()
