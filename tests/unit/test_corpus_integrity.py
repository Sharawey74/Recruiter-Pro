...
def vocabulary():
    ...
    families = raw.get("families") or {k: v for k, v in raw.items() if isinstance(v, dict)}
...
def test_skill_coverage_is_total(jobs, vocabulary):
    ...
    mentions = [s for job in jobs for s in (job.get("required_skills") or [])]
    recognised = [s for s in mentions if s in vocabulary]
    coverage = len(recognised) / len(mentions)

    jobs_with_nothing = sum(
        1 for job in jobs if not any(s in vocabulary for s in (job.get("required_skills") or []))
    )
...
def test_descriptions_do_not_restate_the_skill_list(jobs):
    ...
    echoes = [
        ...
        and all(s.lower() in job.get("description", "").lower() for s in job["required_skills"])
    ]
    assert not echoes, f"{len(echoes)} description(s) name every required skill: {echoes[:5]}"
...
def test_descriptions_are_distinct(jobs):
    ...
    assert len(set(descriptions)) == len(
        descriptions
