...
class TestModels:
    ...
    def test_match_result_complete(self):
        ...
        decision = MatchDecision(
            decision=DecisionType.SHORTLIST, confidence=0.88, reason="Excellent technical fit"
        )
    ...
    def test_match_result_to_history_conversion(self):
        ...
        decision = MatchDecision(
            decision=DecisionType.REVIEW, confidence=0.75, reason="Good but needs review"
        )
...
class TestDatabase:
    ...
    def test_get_top_matches(self, temp_db):
        ...
        for i in range(5):
            ...
            decision = MatchDecision(
                decision=decision_type, confidence=0.7 + (i * 0.05), reason=f"Test match {i}"
            )
